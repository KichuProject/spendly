  ...
  useEffect(() => {
    Animated.loop(
      Animated.sequence([
        ...
        Animated.parallel([
          Animated.sequence([
            Animated.timing(opacity, { toValue: 1, duration: duration * 0.4, useNativeDriver: USE_NATIVE_DRIVER }),
            Animated.timing(opacity, { toValue: 0.3, duration: duration * 0.3, useNativeDriver: USE_NATIVE_DRIVER }),
            Animated.timing(opacity, { toValue: 0.85, duration: duration * 0.3, useNativeDriver: USE_NATIVE_DRIVER }),
          ]),
          Animated.sequence([
            Animated.timing(translateY, { toValue: -28, duration: duration * 0.5, useNativeDriver: USE_NATIVE_DRIVER }),
            Animated.timing(translateY, { toValue: 18, duration: duration * 0.5, useNativeDriver: USE_NATIVE_DRIVER }),
          ]),
        ]),
      ])
    ).start();
    ...
    Animated.loop(
      Animated.sequence([
        ...
        Animated.parallel([
          Animated.sequence([
            Animated.timing(opacity, { toValue: 1, duration: duration * 0.4, useNativeDriver: USE_NATIVE_DRIVER }),
            Animated.timing(opacity, { toValue: 0, duration: duration * 0.6, useNativeDriver: USE_NATIVE_DRIVER }),
          ]),
          Animated.sequence([
            Animated.timing(scale, { toValue: 1.4, duration: duration * 0.5, useNativeDriver: USE_NATIVE_DRIVER }),
            Animated.timing(scale, { toValue: 0.5, duration: duration * 0.5, useNativeDriver: USE_NATIVE_DRIVER }),
          ]),
        ]),
      ])
    ).start();
...
export default function LiquidBackground({ children, style, simplified = Platform.OS === 'web' }) {
  return (
